...
// Self-contained, lightweight SVG icon builder matching exact Lucide SVG specifications
  ...
  const Component = forwardRef((
    ...
    return (
      <svg
        ...
        className={`lucide lucide-${iconName.replace(/([a-z0-9])([A-Z])/g, "$1-$2").toLowerCase()} ${className}`.trim()}
        style={style}
        {...rest}
      >
        {iconNode.map(([tag, attrs], idx) =>
          createElement(tag, { ...attrs, key: attrs.key || idx })
        )}
        {children}
      </svg>
    );
